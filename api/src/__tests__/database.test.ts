...
test('UserType type must be unique',
...
test('A ticket should not have two records with the same userId and eventId',
    async () => {
      await expect(
...
test('A user should not be able to purchase the same ticket twice',
    async () => {
      await expect(
          prisma.ticket.create({
            data: {
